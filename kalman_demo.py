...
if __name__ == '__main__':
    ...
    kalman.measurementNoiseCov = np.array([[0.1, 0], [0, 0.1]], np.float32) * 0.01
    '''
    ...
    for i in range(len(pos)):
        ...
        y = kalman.predict()
        predeictPt.append([float(y[0]), float(y[1])])
        cor.append([float(x[0]), float(x[1])])
        ...
        # print('=' * 30)
    predeictPt = np.array(predeictPt)
    #print(predeictPt)
    predeictPt = predeictPt.reshape(5, 2)
    cor = np.array(cor)
    print("pos:", pos)
    print("cor:", cor)
    print("pre:", predeictPt)

    plt.figure()
    plt.scatter(pos[:, 0], pos[:, 1])
    plt.scatter(predeictPt[:, 0], predeictPt[:, 1])
